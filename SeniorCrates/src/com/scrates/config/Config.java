...
public abstract class Config {
	...
	public Config(String name, double version) {
		...
		this.base = SeniorCrates.getInstance();
	...
	public File backup() {
		...
		SeniorCrates.Log("Backing up...");
		
		if(backups.mkdirs() || backups.exists()) {

			SeniorCrates.Log("created");
			...
			if(!this.getConfig().exists()) return null;
				...
				if(success) {
					SeniorCrates.Log("&a" + this.name + ".yml has been backed up!");	
				}else {
					SeniorCrates.Log("&cCouldnt backup!");
				}
			
				return tempConfig;
			
		}else if(!backups.exists()) {
			SeniorCrates.Log("&cCouldnt create backup folder!");
		}
